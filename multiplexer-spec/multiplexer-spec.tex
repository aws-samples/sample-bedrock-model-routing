...
\cls{TierEscalationConfig} & Record & Opt-in config: \fld{enabled}, \fld{escalationTier} (``reserved'' or ``priority'') \\
\cls{ServiceTierType} & Type alias & ``reserved'' $|$ ``priority'' $|$ ``default'' $|$ ``flex'' \\
\bottomrule
\end{tabular}
...
\medskip
\noindent\textbf{Delegate interface} --- The Request Handler communicates with the Multiplexer through a narrow interface of seven operations: \fn{selectModel}, \fn{getModel}, \fn{reportOutcome}, \fn{emitEvent}, \fn{classifyRefusal}, the \fld{refusalRetryEnabled} flag, and the \fld{tierEscalationConfig} property. This decouples the retry logic from the orchestrator.
...
\begin{enumerate}[nosep]
  ...
  \begin{enumerate}[nosep,label=\alph*.]
    ...
    \end{enumerate}
    \item On \op{RateLimit} error $\to$ attempt tier escalation (step f); if not available or fails, report outcome, add model to skipped, increment retry, continue.
...
\medskip
\noindent\textbf{Service tier escalation} (opt-in, \fn{attemptTierEscalation}):

When \fld{tierEscalation.enabled} is true, the Request Handler retries the \emph{same} model at a higher service tier before falling back to a different model on throttling errors. This adds a resilience dimension that exploits tier-level capacity before exhausting cross-model failover.

\begin{enumerate}[nosep]
  \item On \op{RateLimit} from a model at the default (Standard) tier, check if tier escalation is configured and has not already been attempted for this model in this request.
  \item If available: emit \texttt{tier-escalation} event, call \fn{model.invoke}(input, serviceTier=escalationTier).
  \item On success $\to$ run refusal detection (if enabled), emit \texttt{tier-escalation-success}, report outcome, return response.
  \item On failure $\to$ emit \texttt{tier-escalation-failure}, fall through to cross-model failover (skip model, try next).
\end{enumerate}

\noindent\textbf{Escalation budget:} one attempt per model per request. Tier escalation does not consume the cross-model retry count. The \fld{tierEscalatedModels} set tracks which models have already attempted escalation for the current request.

\noindent\textbf{Escalation tier:} determined by the \fld{tierEscalation.escalationTier} config --- ``reserved'' if the customer has a capacity reservation (separate quota, 99.5\% uptime target), ``priority'' if not (requests prioritized over Standard/Flex at a price premium).
...
\fn{invoke} & input, abortSignal?, serviceTier? $\to$ \{response, outcome\} & Stamp modelId (and optionally serviceTier), check breaker, execute with timeout \\
...
\begin{enumerate}[nosep]
  ...
  \item Construct a Converse API call with $\{\ldots\text{input}, \text{modelId}\}$. If \fld{serviceTier} is provided, stamp $\{\text{serviceTier}: \{\text{type}: \text{serviceTier}\}\}$ onto the request.
...
\texttt{tier-escalation} & (modelId, fromTier, toTier) \\
\texttt{tier-escalation-success} & (modelId, tier) \\
\texttt{tier-escalation-failure} & (modelId, tier, error) \\
\bottomrule
\end{tabular}
...
Tier escalation config & \fld{enabled} required (boolean); when true, \fld{escalationTier} required (``reserved'' or ``priority'') \\
\bottomrule
\end{tabular}
...
\begin{lstlisting}
  ...
  //    f. On RateLimit: [if tierEscalation enabled] attempt tier escalation:
  //       -> emit 'tier-escalation' event
  //       -> model.invoke(input, serviceTier=escalationTier)
  //       -> on success: emit 'tier-escalation-success', return response
  //       -> on failure: emit 'tier-escalation-failure', skip model, retry next
  //       [if not enabled or already attempted] skip model, retry with next
  //    g. On CircuitOpen or Refusal: skip model, retry with next
  //    h. On FailFast: raise to caller immediately
